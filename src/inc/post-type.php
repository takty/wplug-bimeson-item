...
function initialize_post_type( string $url_to ) {
	...
	register_post_type( $inst::PT, [
		'label'         => _x( 'Publication', 'post type', 'bimeson_post' ),
		'labels'        => [],
		'public'        => true,
		'show_ui'       => true,
		'menu_position' => 5,
	...
	if ( is_admin() ) {
		...
		add_action( 'admin_menu',             '\wplug\bimeson_post\_cb_admin_menu_post_type' );
		add_action( 'save_post_' . $inst::PT, '\wplug\bimeson_post\_cb_save_post_post_type' );

		if ( _is_the_post_type() ) {
			add_action( 'admin_enqueue_scripts', function () use ( $url_to ) {
				wp_enqueue_style( 'wplug-bimeson-post-field', $url_to . '/assets/css/field.min.css' );
			} );
		}
...
function _cb_wp_loaded() {
	$inst = _get_instance();
	$cs = [ 'cb', 'title' ];
	$cs[] = [ 'label' => _x( 'Published date', 'post type', 'bimeson_post' ), 'name' => $inst::IT_DATE, 'width' => '10%', 'value' => 'esc_html' ];
	foreach ( get_root_slugs() as $taxonomy ) {
		$cs[] = [ 'name' => root_term_to_sub_tax( $taxonomy ), 'width' => '14%' ];
	}
	$cs[] = 'date';
	$cs[] = [ 'label' => _x( 'Import from', 'post type', 'bimeson_post' ), 'name' => $inst::IT_IMPORT_FROM, 'width' => '10%', 'value' => 'esc_html' ];
...
function _cb_admin_menu_post_type() {
	...
	\add_meta_box( 'bimeson_mb', _x( 'Publication data', 'post type', 'bimeson_post' ), '\wplug\bimeson_post\_cb_output_html_post_type', $inst::PT, 'side', 'high' );
}
...
function _cb_output_html_post_type() {
	...
	output_input_row( _x( 'Published date', 'post type', 'bimeson_post' ), $inst::IT_DATE, $date );
	output_input_row( 'DOI', $inst::IT_DOI, $doi );
	output_input_row( _x( 'Link URL', 'post type', 'bimeson_post' ), $inst::IT_LINK_URL, $link_url );
	output_input_row( _x( 'Link title', 'post type', 'bimeson_post' ), $inst::IT_LINK_TITLE, $link_title );
}
